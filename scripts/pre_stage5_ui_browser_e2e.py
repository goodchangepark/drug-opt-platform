...
def main():
    ...
    try:
        ...
        if driver.execute_script("return getComputedStyle(document.querySelector('.sidebar-body')).display") == "none":
            raise AssertionError("Mobile primary navigation did not expand")
        click_button(driver, "Close")
        driver.set_window_size(1900, 1800)
        checks.extend(["Platform overview and scientific module registry", "Responsive collapsible primary navigation"])
        ...
        WebDriverWait(driver, 30).until(lambda d: "RESEARCH PORTFOLIO" in d.page_source and PROJECT_NAME in d.page_source)
        click_button(driver, "Optimization")
        WebDriverWait(driver, 30).until(lambda d: "PROJECT OPTIMIZATION" in d.page_source and "Optimization Workspace" in d.page_source)
        click_button(driver, "Settings")
        WebDriverWait(driver, 30).until(lambda d: "Project Settings" in d.page_source)
        click_button(driver, "Help")
        WebDriverWait(driver, 30).until(lambda d: "Platform Workflow" in d.page_source and "Where scientific functions live" in d.page_source)
        click_button(driver, "New Project")
        WebDriverWait(driver, 30).until(lambda d: "Create New Project" in d.page_source and "Typical Workflow" in d.page_source)
        click_button(driver, "Dashboard")
        WebDriverWait(driver, 30).until(lambda d: "Available Scientific Modules" in d.page_source)
        checks.append("Six-item global sidebar navigation and project-scoped optimization/settings")
